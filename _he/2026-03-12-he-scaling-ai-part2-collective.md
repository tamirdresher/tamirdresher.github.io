...
ב[חלק 1](/he/2026/03/11/scaling-ai-part1/), הקמתי את ה-Squad הראשון שלי על פרויקט פנימי שנקרא provisioning wizard — עם צוות בסגנון Matrix: Morpheus, Trinity, Switch, Dozer (Star Trek כבר היה תפוס בריפו האישי שלי) — וצפיתי בהם טוחנים 52 משימות Planner במקביל. מדהים. ואז עשיתי מה שכל מהנדס נלהב עושה: הקמתי Squad על ריפו שני.
...
- **צוות** — מומחיות תחומית שחוצה מספר ריפוים. "ה-.NET SDKs שלנו עוקבים אחרי הדפוס הזה." "ADRs נמצאים ב-`docs/decisions/`."
...
- `.roo/context/` — 8 קבצי הקשר שמכסים את ארכיטקטורת ה-SDK, דפוסי deployment, מוסכמות שמות, טיפול בשגיאות ומבנה טסטים
