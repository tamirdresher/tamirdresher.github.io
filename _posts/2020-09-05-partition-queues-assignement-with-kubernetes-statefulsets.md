...
date: 2020-09-05
---
Partitioning a domain is a useful way to achieve scalability of a system. The idea behind partitioning is that instead putting everything in a single place, you divide the dataset or the work to multiple places based on some attribute, which usually is the identifier of the entity.
...
One of the requirements that you might need to fulfill in partitioned environment is having a single master consumer per each partition. This is usually needed to achieve consistency and an ordered execution.

For example, if the system you are working on involves financial data, it makes perfect a lot of sense to process all the transactions related to a specific account in the order they were registered.
...
In Kuberenets, a statefulSet is a type of deployment strategy that maintains a sticky identity for each of the Pods.
...
When a pod crash or being rescheduled, Kubernetes will take of all of complexity of reviving it and reassign it to the correct identity.
...
These attributes of the statefulSet makes it easier for us to implement our solution/

  

## Implementing the Partition Queues Assignement with StatefulSet

Suppose we have a system that work with a traditional message broker such as RabbitMQ, ActiveMQ, SQS, Azure Storage Queues or alike.
We have a queue named "OrderEvents" that holds event about orders we wish to partition to P partitions.
      ...
      containers:
        - name: consumer
          ...
          resources:
            {}
```
The important pieces here is that the _kind_ attribute was set to _StatefulSet_ and that the amount of replicas was set to 2.
Also, I'm setting several environment variables so the consumer would know the prefix for the queues and how many partitions exist.  
...
When Kubernetes schedule a pod as part of statefulSet, it assign its hostname in this format: $(statefulset name)-$(ordinal)
...
```
nnote: the extraction can also be made outside of the code and as part of the container init script (https://kubernetes.io/docs/concepts/workloads/pods/init-containers/)
...
1. The consumer application will read the value directly from Kuberenets
...
Imagine the scenario that a consumer was reading messages from one of the queues and start processing them, and before finishing, the number of replicas increased.
...
The idea is not complex, before the consumer connect to the queue, it tries to take a hold of the distributed lock, when the ownership is revoked, the consumer frees the lock.
...
Because this all exercise started with the question of how we can assign RabbitMQ queues to a set of consumers, it's only natural to try and leverage RabbitMQ capabilities.
...
This is a perfect fit to our problem. I can let the new consumer subscribe to the queue and rest assured that only after the previous consumer finish, messages will start to flow to the new one.

Here is how the consumer code that handle a single partition looks like:
