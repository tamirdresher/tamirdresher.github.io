...
Our solution was a three-layer guard in `ralph-watch.ps1` (lines 35–71):
...
But we also hit an accidental comedy along the way. When creating the channels, we discovered there were **two** teams — "Squad" (Brady's team) and "squads" (my team). The notification ended up in the wrong "Squad" team's new channel. We had to delete it and recreate it under "squads." In distributed systems, this is the **service discovery problem** — you need to resolve the correct endpoint, and names that look similar can route traffic to the wrong destination. DNS taught us this decades ago.
...
But `merge=union` has limits. It doesn't help when agents need to write **structured** decisions — like team architecture decisions that need formatting, context, and cross-references. So we built the **drop-box pattern**.
...
Douglas Guncet — a Solution Architect on Microsoft's Accelerate Factory team — reached out about scaling Squad to **100+ parallel clients**. His email crystallized the problem:
